...
public readonly ref struct MapFeatureData
{
    ...
    public Dictionary<MyKeyName, string> Properties { get; init; }
}
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<MyKeyName, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        if (GetEnumId.getEnumId(key.ToString()) != -1)
                        {
                            properties.Add((MyKeyName)GetEnumId.getEnumId(key.ToString()), value.ToString());
                        }
                    }
