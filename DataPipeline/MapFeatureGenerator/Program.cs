...
public static class Program
{
        ...
        foreach (var (tileId, _) in mapData.Tiles)
        {
            ...
            foreach (var way in mapData.Ways)
            {
                var featureData = new FeatureData
                {
                    ...
                    PropertyKeys = (totalPropertyCount, new List<Mapster.Common.MemoryMappedTypes.MyKeyName>(way.Tags.Count)),
                ...
                foreach (var tag in way.Tags)
                {
                    ...
                    featureData.PropertyKeys.keys.Add((Mapster.Common.MemoryMappedTypes.MyKeyName)Mapster.Common.MemoryMappedTypes.GetEnumId.getEnumId(tag.Key));
                ...
                foreach (var nodeId in way.NodeIds)
                {
                    ...
                    {
                        if (!featureData.PropertyKeys.keys.Contains((Mapster.Common.MemoryMappedTypes.MyKeyName)Mapster.Common.MemoryMappedTypes.GetEnumId.getEnumId(key)))
                        {
                            featureData.PropertyKeys.keys.Add((Mapster.Common.MemoryMappedTypes.MyKeyName)Mapster.Common.MemoryMappedTypes.GetEnumId.getEnumId(key));
            ...
            foreach (var (nodeId, node) in mapData.Nodes.Where(n => !usedNodes.Contains(n.Key)))
            {
                ...
                var featurePropKeys = new List<Mapster.Common.MemoryMappedTypes.MyKeyName>();
                ...
                for (var i = 0; i < node.Tags.Count; ++i)
                {
                    ...
                    featurePropKeys.Add((Mapster.Common.MemoryMappedTypes.MyKeyName)Mapster.Common.MemoryMappedTypes.GetEnumId.getEnumId(tag.Key));
            ...
            foreach (var t in featureIds)
            {
                ...
                {
                    Mapster.Common.MemoryMappedTypes.MyKeyName k = featureData.PropertyKeys.keys[i];
    ...
    private struct FeatureData
    {
        ...
        public (int offset, List<Mapster.Common.MemoryMappedTypes.MyKeyName> keys) PropertyKeys { get; init; }
